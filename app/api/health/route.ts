...
export async function GET(request: Request) {
  ...
  const checks = {
    ...
    googleWorkspace: "Not configured",
  ...
  try {
    ...
    else await env.FILES.health();
    checks.fileStorage = "Healthy";
  } catch {
    /* report unavailable */
  }
  try {
    const google = await googleConnectionStatus(request, auth.user.id);
    checks.googleWorkspace = google.connected
      ? "Connected"
      : google.configured
        ? "Ready to connect"
        : "Not configured";
  } catch {
    checks.googleWorkspace = "Unavailable";
  }
  if (
    process.env.REALTIME_URL &&
    process.env.REALTIME_TOKEN_SECRET &&
    process.env.REALTIME_TOKEN_SECRET.length >= 32
  )
    checks.realtime = "Gateway configured";
  const healthy =
    checks.database === "Healthy" &&
    checks.fileStorage === "Healthy" &&
    !["Not configured", "Unavailable"].includes(checks.googleWorkspace);
