...
export function StatusPill({ value }: { value: string }) {
  ...
  const tone = lower.includes("approved") || lower.includes("complete") || lower.includes("operational") || lower.includes("active") || lower.includes("confirmed") || lower.includes("available") || lower.includes("verified") || lower.includes("done") ? "green"
    : lower.includes("reject") || lower.includes("restricted") || lower.includes("outage") ? "red"
    : lower.includes("pending") || lower.includes("await") || lower.includes("due") || lower.includes("maintenance") || lower.includes("review") || lower.includes("cover") ? "amber" : "blue";
...
export function SvgIcon({ name, className = "", size = 18 }: { name: string; className?: string; size?: number }) {
  ...
  switch (name) {
    ...
    case "operations":
    case "truck":
      return <svg style={s} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>;
    case "settings":
    case "gear":
